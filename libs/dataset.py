...
def load_spectrogram(path, image_size):
    ...
    spectrogram = spectrogram.resize((image_size, image_size), resample=Image.ANTIALIAS)
    spectrogram_data = np.asarray(spectrogram, dtype=np.uint8).reshape(image_size, image_size, 3)
...
def create_dataset_from_slices(spectrograms_per_genre, genres, slice_size, validation_ratio, test_ratio):
    ...
    train_x = np.array(x[:train_len]).reshape([-1, slice_size, slice_size, 3])
    train_y = np.array(y[:train_len])
    validation_x = np.array(
        x[train_len:train_len + validation_len]).reshape([-1, slice_size, slice_size, 3])
    validation_y = np.array(y[train_len:train_len + validation_len])
    test_x = np.array(x[-test_len:]).reshape([-1, slice_size, slice_size, 3])
